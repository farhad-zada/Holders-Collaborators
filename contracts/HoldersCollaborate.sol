...
contract Collaborators is Initializable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  mapping(address => mapping(uint8 => mapping(address => uint256 amount))) private boostings;
  // level => LevelDetails
  ...
  modifier boostingAvailable(TokenData memory tokenData, uint256 amount) {
    ...
    if (boostings[tokenData.token][LEVEL][msg.sender] + amount > maxOf) revert Collaborate__OverMax();

    if (boostings[tokenData.token][LEVEL][msg.sender] + amount < minOf) revert Collaborate__UnderMin();

    uint256 threshold = _thresholdOfToken(tokenData, LEVEL);

    if (totalBoostToken[tokenData.token][LEVEL] >= threshold) revert Collaborate__BoostingComplete();

    if (totalBoostToken[tokenData.token][LEVEL] + amount > threshold) revert Collaborate__OverFull();

    if (!interval.intervalSet) revert Collaborate__IntervalNotSet();

    if (interval.startsAt > block.timestamp) revert Collaborate__NotActive();
  ...
  modifier boostingComplete(uint8 _level) {
    if (!(_boostingComplete(_level))) revert Collaborate__BoostingNotComplete();
    _;
  }
  ...
  function setImperatives(
    ...
    imperatives = Imperatives(commonMaxBoost, commonMinBoost, whitelistMaxBoost, whitelistMinBoost, true);
  }

  function setInterval(uint256 startsAt, uint256 endsAt) public admin {
    require(startsAt < endsAt, "Collaborate: Interval must be gt 0");
    interval = Interval(startsAt, endsAt, true);
  }

  function setWhitelist(address token, address[] memory accounts, bool[] memory statuses) public admin {
    require(accounts.length == statuses.length, "Collaborate: mismatch of values");
  ...
  function setLevelsDetails(
    ...
    if (levels.length != thresholds.length || levels.length != rewardPercents.length)
      revert Collaborate__LengthMismatch();
    for (uint i; i < levels.length; i++) {
      levelDetails[levels[i]] = LevelDetails(thresholds[i], rewardPercents[i]);
    }
  }
  ...
  function withdrawBoostingAndSetClaims(address token, uint8 level) public boostingComplete(level) nonReentrant {
    (, TokenData memory tokenData) = _tokenExists(token);

    IERC20Upgradeable _boostedToken = IERC20Upgradeable(token);

    (uint256 amount, uint256 baseReward) = _calcRewardInBase(tokenData, msg.sender, level);
  ...
  function _beforeBoostAndUpgrade(
    ...
    boostings[tokenData.token][LEVEL][msg.sender] = boostings[tokenData.token][LEVEL][msg.sender] + amount;
  ...
  function _tokenExists(address token) internal view returns (bool exists, TokenData memory tokenData) {
    for (uint i; i < tokens.length; i++) {
      if (tokens[i].token == token) {
  ...
  function _minMaxOf(TokenData memory _token, address account) internal view returns (uint256 maxOf, uint256 minOf) {
    if (whitelist[_token.token][account]) {
      minOf = (imperatives.whitelistMinBoost * base) / _token.baseRelative;
      maxOf = (imperatives.whitelistMaxBoost * base) / _token.baseRelative;
    } else {
  ...
  function _thresholdOfToken(TokenData memory tokenData, uint8 _level) internal view returns (uint256) {
    return (levelDetails[_level].threshold * base) / tokenData.baseRelative;
  }

  function _calcRewardInToken(TokenData memory tokenData, uint256 baseReward) internal pure returns (uint256 reward) {
  ...
  function _boostingComplete(uint8 _level) internal view returns (bool) {
    for (uint i; i < tokens.length; i++) {
  ...
  function _boostingAvailable(TokenData memory tokenData, uint256 amount) internal view returns (bool) {
    if (status != Status.ACTIVE) return false;
    (uint256 maxOf, uint256 minOf) = _minMaxOf(tokenData, msg.sender);
    if (boostings[tokenData.token][LEVEL][msg.sender] + amount > maxOf) return false;
    if (boostings[tokenData.token][LEVEL][msg.sender] + amount < minOf) return false;
    uint256 threshold = _thresholdOfToken(tokenData, LEVEL);
    if (totalBoostToken[tokenData.token][LEVEL] >= threshold) return false;
    if (totalBoostToken[tokenData.token][LEVEL] + amount > threshold) return false;
    if (!interval.intervalSet) return false;
    if (interval.startsAt > block.timestamp) return false;
    if (interval.endsAt < block.timestamp) return false;
    return true;
  }

  // -> R

  function canBoost(address token, uint256 amount) public view returns (bool) {
    (bool exists, TokenData memory tokenData) = _tokenExists(token);
    if (!exists) return false;
    return _boostingAvailable(tokenData, amount);
  }
  ...
  function getTokenTreshold(address token, uint8 level) public view returns (uint256) {
    (, TokenData memory tokenData) = _tokenExists(token);
    uint256 threshold = _thresholdOfToken(tokenData, level);
    return threshold;
  }

  function getMyBoosting(address token, uint8 level) public view returns (uint256) {
    return boostings[token][level][msg.sender];
  }

  function getBoostingComplete(uint8 level) public view returns (bool) {
    return _boostingComplete(level);
  }
  ...
  function getInterval() public view returns (uint256 startsAt, uint256 endsAt) {
  ...
  function getImperatives() public view returns (uint256, uint256, uint256, uint256) {
    return (
  ...
  function getLevelDetails(uint8 level) public view returns (uint256, uint256) {
    return (levelDetails[level].threshold, levelDetails[level].rewardPercent);
  }

  function getTotalBoost(address token, uint8 level) public view returns (uint256) {
    return totalBoostToken[token][level];
  }

  function getWhitelist(address token, address account) public view returns (string memory name, bool _status) {
    (, TokenData memory tokenData) = _tokenExists(token);
    name = tokenData.name;
    _status = whitelist[token][account];
  }
  ...
  function getMaxMin(address _token) public view returns (uint256 maxOf, uint256 minOf) {
